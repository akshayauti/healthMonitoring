...
def train_classifier():
    data_dir="f:/BE_project/facemask/data"
...
def get_name(image):
    # return "Akshay Auti"
    def draw_boundary_img(img,classifier,scaleFactor,minNeighbors,color,text,clf):
        gray_image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        features = classifier.detectMultiScale(gray_image,scaleFactor,minNeighbors)
    ...
    def recognize_img(img,clf,faceCascade):
        name = draw_boundary_img(img,faceCascade,1.1,10,(255,255,255),"Face",clf)
        return name

    faceCascade=cv2.CascadeClassifier("haarcascade_frontalface_default.xml")
    clf = cv2.face.LBPHFaceRecognizer_create()
    clf.read("classifier.xml")
    name = recognize_img(image,clf,faceCascade)
    return name
...
def generate_dataset():
    if(t1.get()=="" or t2.get()=="" or t3.get()=="" or t4.get()==""):
        ...
        for x in myresult:
            id+=1
        sql="insert into my_table(id,Name,Age,Address,Email) values(%s,%s,%s,%s,%s)"
